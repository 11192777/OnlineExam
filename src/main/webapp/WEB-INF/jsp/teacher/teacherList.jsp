...
<body>
			...
			<div class="col-sm-3 col-md-2 sidebar">
				<div class="tree">
					<ul style="padding-left:0px;" class="list-group">
						...
						<li class="list-group-item "><span><i class="glyphicon glyphicon glyphicon-user"></i> 用户管理 <span class="badge" style="float:right">3</span></span>
							<ul style="margin-top:10px;">
								<li style="height:30px;"><a href="${APP_PATH}/student/studentList"><i class="glyphicon glyphicon-user"></i> 学生管理</a></li>
								<li style="height:30px;"><a href="${APP_PATH}/teacher/teacherList" style="color: red;"><i class="glyphicon glyphicon-king"></i> 教师管理</a></li>
							</ul></li>
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title">
							<i class="glyphicon glyphicon-th"></i> 教师列表
						</h3>
					</div>
					<div class="panel-body">
						<form class="form-inline" role="form" style="float: left;">
							<div class="form-group has-feedback">
								<div class="input-group">
									<div class="input-group-addon">查询教师</div>
									<input class="form-control has-success" id="queryText" type="text" placeholder="输入教师账号">
								</div>
							</div>
						...
						<button type="button" class="btn btn-primary" style="float: right;" onclick="window.location.href='${APP_PATH}/teacher/teacherList/addTeacher'">
	...
	<script type="text/javascript">
		...
		function deleteUser(userId) {
			layer.confirm("您确认要删除用户，" + userId + "吗？", {icon : 3,title : '提示'
			}, function(cindex) {
				$.ajax({
					type : "POST",
					url : "${APP_PATH}/teacher/teacherList/doDeleteTeacher",
					data : {
						"userId" : userId
					},
					beforeSend : function() {
					...
					success : function(result) {
						layer.close(index);
						if ( result.success ) {
							layer.msg("删除成功", {time : 2000,icon : 6,}, function() {pageQuery();});
						} else {
		...
		function pageQuery( queryText ) {
			$.ajax({
				type : "POST",
				url : "${APP_PATH}/teacher/teacherList/doPageQuery",
				data : { 
					"queryText" : queryText
				},
				beforeSend : function() {index = layer.load(2, {time : 10 * 1000});
				},
				success : function(result) {
					layer.close(index);
					...
					for (var i = 0; i < datas.length; i++) {
						...
						context += '	<td>' + data.teaName + '</td>';
						context += '	<td>' + data.teaSex + '</td>';
						context += '	<td>' + data.teaMajor + '</td>';
						context += '	<td>' + data.teaAge + '</td>';
						context += '	<td>' + data.teaPhone + '</td>';
						context += '	<td>';
